...
def create_knn_graph(
    points, probs, k, lower_threshold=None, upper_threshold=None
):
    """# noqa
    ...
    nearest_neighbors = np.argsort(dists, axis=1)[
        :, 1 : k + 1  # noqa
    ]  # slice out the 0th, which is the point itself
    ...
    # Add edges for mutual nearest neighbors within optional distance threshold
...
def graph_pruning(
    graph, score_threshold, angle_threshold, sarcs_angle, remove_edges=True
):
    """# noqa
...
def myofibril_extension(graph, zdiscs, probs, prob_thresh=0.5):
    """# noqa
    ...
    for _ in range(5):
        ...
        angle_threshold = (
            180 - 1.75 * 90
        )  # if referencing sg.config._angle_threshold = 1.75
        min_distance = (
            7  # e.g. "np.mean(dists) - 2 * np.std(dists)" used previously
        )
        ...
        for myo in myofibrils:
            ...
            for end_node in end_nodes:
                ...
                pred_sarc_angle_end = (
                    180.0 * np.arccos(np.clip(sarc_cosine, -1, 1)) / np.pi
                )

                # Filter by length, angle, probability
                if (
                    pred_sarc_length < min_length
                    or pred_sarc_length > max_length
                    or pred_sarc_angle_end > angle_threshold
                    or probs[target_node] < prob_thresh
                ):
                    continue
                ...
                if nodes_degree[target_node] == 1:
                    ...
                    angle_tgt = (
                        ...
                        * np.arccos(
                            ...
                            / (
                                np.linalg.norm(pred_sarc)
                                * np.linalg.norm(target_sarc)
                                + 1e-8
                            )
                        )
                        / np.pi
                ...
                pred_zdisc_2 = (
                    pred_zdisc_2 + graph.nodes[target_node_2]["pos"]
                ) / 2
                ...
                if (
                    pred_sarc_2_length < min_length
                    or pred_sarc_2_length > max_length
                    or angle_sarc_2 > angle_threshold
                    or probs[target_node_2] < prob_thresh
                ):
                    continue
                ...
                if nodes_degree[target_node_2] == 0:
                    ...
                    graph.nodes[new_idx]["prob"] = graph.nodes[target_node][
                        "prob"
                    ]
                ...
                if nodes_degree[target_node_2] == 1:
                    ...
                    target_sarc_2 = (
                        graph.nodes[tgt_2_neighbor]["pos"]
                    ...
                    if angle_tgt_2 < angle_threshold:
                        ...
                        graph.add_node(new_idx)
                        graph.nodes[new_idx]["pos"] = pred_zdisc
                        graph.nodes[new_idx]["prob"] = graph.nodes[
                            target_node
                        ]["prob"]
                        graph.add_edge(end_node, new_idx)
                        nodes_degree[end_node] += 1
