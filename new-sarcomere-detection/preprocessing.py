...
def load_sample_data(
    ...
    contours = np.load(f"{zdiscs_dir}/contours-{sample_id}.npy", allow_pickle=True)
...
def correct_zdiscs(
    ...
    for contour_id, contour in enumerate(contours):
        ...
        roi_masked = (mask[y : y + h, x : x + w].astype(np.float32)
                      * raw_padded[y : y + h, x : x + w] / 255.0)
        ...
        if np.max(roi_masked) != np.min(roi_masked):
            roi_masked = (roi_masked - np.min(roi_masked)) / (np.max(roi_masked) - np.min(roi_masked))
        ...
        bounds = [(0, h - 1), (0, w - 1)]
        init_points = np.random.uniform([0, 0], [h - 1, w - 1], (initial_points, 2))

        # Collect all local optima that meet the intensity threshold
        optima = set()
        for pt in init_points:
            res = minimize(interpolator_function, pt, bounds=bounds, method='L-BFGS-B')
            val_at_min = interpolator_function(res.x)
            # Because we used negative, "res.success" means we found a local minima
        ...
        if len(optima) == 0:
            ...
            for label in set(labels):
                if label != -1:
                    ...
                    recorded_optima.append((int(cluster_center[0]), int(cluster_center[1])))
            ...
            if contour_probs:
                ...
                contour_probs = p * np.array(contour_probs) / np.min(contour_probs)
