...
from graph_operations import create_knn_graph, get_angle, update_scores, graph_pruning
...
def score_method_1(graph, sg_config, sarc_vector_func, sarc_score_func):
    ...
    """
    # 1) Use sarc_vector_func & sarc_score_func to compute an initial edge score
    score_graph(graph, sg_config, sarc_vector_func, sarc_score_func)

    # 2) Scale as in your snippet: graph[u][v]["score"] /= 3
    for (u, v) in graph.edges:
        graph[u][v]["score"] = (graph[u][v]["score"] / 3 + (graph.nodes[u]["prob"] + graph.nodes[v]["prob"]) / 2) / 2
...
def score_method_3(graph, eta, angle_threshold_deg, iter_num, get_angle_func):
    ...
    for node in graph.nodes:
        ...
        for i in range(len(neighbors)):
            for j in range(i + 1, len(neighbors)):
                ...
                e1_id = edges_id[(min(neighbors[i], node), max(neighbors[i], node))]
                e2_id = edges_id[(min(node, neighbors[j]), max(node, neighbors[j]))]
    ...
    for _ in range(iter_num):
        for (u, v) in list(graph.edges):
            update_scores(graph, u, v, eta, angle_threshold_deg, edges_id, angles_matrix)
            update_scores(graph, v, u, eta, angle_threshold_deg, edges_id, angles_matrix)

    # Combine node-specific scores into a final 'score'
    max_score = 0.0
    for edge in graph.edges:
        u, v = edge
        s = (graph.edges[edge][f"score_{u}"] + graph.edges[edge][f"score_{v}"]) / 2.0
...
def probabilistic_ensemble(
    ...
    for (u, v) in base_graph.edges:
        ...
        # Compute product of s_i and product of (1 - s_i)
        # By default, each edge's prior is zdisc_prior, so we do (N-1) in exponent if you want that formula
        # or adapt to your snippet's partial combos, etc.
        if len(scores) == 0:
        ...
        # but you can adapt the exponent. If you have N methods, you might want zdisc_prior^N, etc.
        N = len(scores)
        a = prod_s / (zdisc_prior ** (N - 1)) if N > 1 else prod_s
        b = prod_1_minus_s / ((1 - zdisc_prior) ** (N - 1)) if N > 1 else prod_1_minus_s
...
def build_and_score_all(zdiscs_corrected, new_probs, sg, configs):
    ...
    # Possibly define clip ranges if you want to clamp method #2 scores to [0.4, 0.6], etc.
    # E.g. (graph, min, max)
    scoring_graphs = [
    ...
    probabilistic_ensemble(
        base_graph=graph_ensemble,
        other_graphs=scoring_graphs,
        zdisc_prior=0.5
    )
